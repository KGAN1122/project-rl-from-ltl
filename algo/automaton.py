...
class OmegaAutomaton:
    ...
    def __init__(self,ltl,auto_type='ldba'):
        ...
        for q in range(len(self.delta)):
            for label, dest in self.delta[q].items():
                acc_info = self.acc[q][label]
                if acc_info != [None] * len(acc_info):
                    print(f"q{q} --{label}--> q{dest}, acc = {acc_info}")
    ...
    def ltl2auto(self,ltl):
        ...
        out=check_output(['/home/gk/tools/rabinizer4/bin/ltl2ldba', '-d', '-e', ltl] if self.auto_type == 'ldba' else ['/home/gk/tools/rabinizer4/bin/ltl2ldba', '-c', ltl])
